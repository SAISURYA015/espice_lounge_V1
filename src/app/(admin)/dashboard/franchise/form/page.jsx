...
const ApplyForm = () => {
  ...
  const fetchData = async () => {
    try {
      const records = await pb.collection("franchise_form").getFullList(
        {
          sort: "-created",
        },
        { requestKey: null }
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this record?")) {
      try {
        await pb.collection("franchise_form").delete(id);
  ...
  return (
    <>
      ...
      <div className="fixed top-0 p-4 bg-gray-100 border-b w-full flex flex-wrap gap-2">
        ...
        <span className="text-gray-600">/</span>
        <a href="/dashboard/franchise/forms">
      ...
      <div className="p-4 mt-14 w-full">
        <div className="flex justify-between items-center mb-3">
          <h2 className="font-semibold text-lg">Franchise (Forms)</h2>
        </div>
