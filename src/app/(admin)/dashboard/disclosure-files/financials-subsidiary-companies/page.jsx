...
const FinancialSub = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingRow) {
        await pb.collection("fin_subsidiary").update(editingRow.id, updateData);
      } else {
        await pb.collection("fin_subsidiary").create(updateData);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      await pb.collection("fin_subsidiary").delete(editingRow.id);
  ...
  return (
    <>
      ...
      <div className="fixed top-0 p-4 bg-gray-100 border-b w-full flex flex-wrap gap-2 z-10">
        ...
        <span className="text-black">Financials Subsidiary Companies</span>
      </div>
